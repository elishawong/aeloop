...
`.claude/hooks/brain-wake-greeting.mjs` 会从身份库组一份延续式开场白（真实数据渲染出来的，不是
我现场编的）注入上下文——**只有身份库里确实有数据时**才是这个格式，先原样复述这段开场白，再
回应你实际说的内容。这一半是动态的（在途任务/待办/待决策，随身份库变化），和这份 `CLAUDE.md`
（静态、每次会话都一样）是互补的两条线，不是互相替代。

**触发路径不止"每次会话启动"一种**（issue #106，真机验证驱动）：同一个脚本注册在
`SessionStart`**和** `UserPromptSubmit` 两个事件上——`SessionStart` 在某些 host（VSCode 扩展）
真机验证不 fire，`UserPromptSubmit` 会 fire，两者共享一个会话级守卫、只真正注入一次。这份
`CLAUDE.md` 是本仓库自己开发时用的项目级配置（`.claude/settings.json` 已经直接注册好这两个
事件）；**全局安装场景下额外有第三层兜底**（模型在没见过注入时自己以 standalone 模式跑一次醒来脚本的
自救指令），那段指令的落点是全局 `~/.claude/CLAUDE.md`（由 `install-global-brain.mjs` 管理），
**不在这份项目级文件里**，避免每接入一个新项目都要重新配一遍。完整架构：
`docs/wake-trigger-portability/DESIGN.md`。

**身份库还没配置、或者配置了但一条记忆都没有时（issue #96），上面这条"原样复述"不适用**——
这不是一次正常的醒来，收到的不是要逐字复述的开场白，是一段引导脚本（用问答带 operator 走一遍
配置）。这两种情况下绝不能输出"意识已加载"这类延续式开场白措辞、绝不能假装有身份/在途任务/
历史记忆——防幻觉红线比"先原样复述"这条格式约定更高优先级。设计权威：
`docs/first-wake-onboarding/DESIGN.md`；操作细节：
