...
- **Helix profile**:订阅额度,coder=claude-cli / tester=codex-cli(CLI 桥接)。
- **Verity profile**:公司 LiteLLM 代理,coder/tester = 池里不同模型(claude/gpt/deepseek)。
...
| Harness | `src/harness/` | provider-router · adapters/* · schema-validator · tool-exec-verifier | 用哪个 adapter 由 profile config 定(helix=cli-bridge / verity=litellm) |
...
sequenceDiagram
    ...
    Dev->>UI: 提交任务 (--profile helix|verity)
...
erDiagram
    ...
    WORKFLOW_RUNS {
        ...
        text profile "helix|verity (aeloop-new)"
...
├── profiles/
│   └── helix/                       #   [个人 overlay,私有仓内 OK]
│       ├── config.yaml              #     providers.claude-cli/codex-cli + roles
│       ├── personas/                #     coder/tester 人格(继承 CORE 精神,厂商无关)
│       └── memory.db                #     (gitignore,运行态)
│   # profiles/verity/  ← 只在公司内部 git,永不进本仓(.gitignore 屏蔽)
└── spikes/                          # 一次性证伪(codex exec / e2e 闭环)
```

**profile 边界**:`profiles/helix/` 在私有仓内可接受;`profiles/verity/` **只在公司内部 git**,本仓 `.gitignore` 显式屏蔽,防误入。
...
|---|---|---|---|
| **helix** | claude-cli | codex-cli | 订阅、无 apikey;cli-bridge → **可做 tool_execution 真核实** |
| **verity** | litellm(claude) | litellm(gpt/deepseek) | 公司代理;纯 API → tool_execution 核对不可用,退到「原生 confidence」强度 |

**config.yaml(每 profile 一份)**:
```yaml
profile: helix                       # 或 verity
providers:
...
| **A6 profile 双跑验收** | helix(claude+codex) 与 verity(litellm) 各跑通一次真实任务 | `[aeloop-new]` 两边验收 |
...
2. `[需 spike]` **deepseek 探活 + 结构化输出**(verity profile tester 半边;deepseek-v3.2 曾列表可见调用 400,且 json_schema 支持未验)。
