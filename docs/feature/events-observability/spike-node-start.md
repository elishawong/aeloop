...
- `compiled.stream(input, { ...cfg, streamMode: ["updates", "tasks"] })` in `runner.ts` — the **one** `compiled.stream()` call site inside `runStreamAndPersistCore()` (shared by both `startRun()` and `resumeRun()`, which both call this same function — not two separate call sites), currently `streamMode: "updates" as const`, becomes an array.
